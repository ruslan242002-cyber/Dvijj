...
*
* РЕДКОСТЬ И ЦЕПОЧКИ (по разбору Kimi) — pickContracts всегда берёт
* только common на старте дня (3 шт., как раньше). После того как игрок
* выполнил ВСЕ 3 common за сегодня, maybeUnlockRareContract() добавляет
* один rare-контракт в список (один раз за день). Legendary — отдельный
...
// getReputationTitle ниже оставлен (реэкспорт из engine/reputation.js)
// для обратной совместимости импортов в других файлах.
