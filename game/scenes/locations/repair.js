...
if (input.startsWith(' Расширить бак')) {
...
if (!sysId) return { reply: { text: hubMessage(state.player), buttons: stationButtons(deps, state.player), imageKey: imageForLocation('station', state.player.faction) }, nextState: { scene: 'station', player: state.player } };
const player = { ...state.player, ship: { ...state.player.ship }, inventory: (state.player.inventory || []).map((i) => ({ ...i })) };
