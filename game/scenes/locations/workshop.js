...
return `${isEq ? ' ' : ' '} [${r.slot === 'weapon' ? 'Оружие' : 'Броня'}] ${r.name}${lvlText} (${statsText(statsAtLevel(r, lvl))})`;
});
...
return `${isEq ? ' ' : ' '} ${a.name} (+${a.bonus} ${a.stat})`;
});
const sections = [];
if (equippedLines.length || notEquippedLines.length) {
sections.push(` Модули (слотов занято ${equipped.length}/${slots}) — экипировать, снять:\n${[...equippedLines, ...notEquippedLines].join('\n')}`);
}
if (gearOwnedLines.length) {
sections.push(` Снаряжение (у тебя) — экипировать, снять, улучшить:\n${gearOwnedLines.join('\n')}`);
}
...
sections.push(` Снаряжение — доступно скрафтить:\n${gearCraftLines.join('\n')}`);
}
const buttons = [];
...
if (upgradeLevelOf(player, id) < MAX_UPGRADE_LEVEL) buttons.push(` ${r.name}`);
}
...
const craftGearMatch = /^ (.+)$/.exec(input);
if (craftGearMatch) {
...
const equipGearMatch = /^ (.+)$/.exec(input);
if (equipGearMatch) {
