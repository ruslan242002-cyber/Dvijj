...
lines.push(` Изучено (слотов занято ${equipped.length}/${slots}${slotGrowthNote}):`);
for (const id of known) {
const skill = PASSIVE_SKILLS[id];
if (!skill) continue;
const isEquipped = equipped.includes(id);
lines.push(` ${isEquipped ? ' ' : ' '} ${skill.name} — ${skill.description}`);
...
return {
reply: { text: ` Ты официально вступил(а) в станцию «${newFaction}». Умения и статы пересчитаны под новую фракцию.`, buttons: stationButtons(deps, player), imageKey: imageForLocation('station', player.faction) },
