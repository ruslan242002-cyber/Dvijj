...
/**
 * Прибытие в узел не всегда означает прибытие на станцию.
 *
 * CITY:
 *   узел города → сдаём tripCargo → station
 *
 * LOCATION:
 *   промежуточный узел → оставляем груз рискованным → продолжаем travel
 */
async function arriveAtNode(deps, player, nodeId, prefixText = '') {
  ...
  return {
    reply: {
      text: `${prefixText}🛰️ Прибытие: ${nodeName}.\n📦 Несданный груз: ${cargo} ед.\n\nКорабль остаётся в открытом космосе. Можно продолжить рейс или вернуться по доступному Тракту.`,
      buttons: ['🚀 Продолжить полёт'],
    },
    nextState: {
...
async function travelScreen(deps, player, prefixText = '') {
  ...
  const node = nodeById(nodeId);
  const activeTracts = deps.tractStore
    ? await deps.tractStore.getActiveTracts()
    : [];
  ...
  if (!routes.length) {
    return {
      reply: {
        text: `${prefixText}📍 ${node?.name || nodeId}\n⛽ Топливо: ${player.ship.fuel}/${player.ship.fuelMax}\n📦 Несданный груз: ${cargo} ед.\n\nОтсюда сейчас нет ни одного доступного маршрута — Тракт разорван в этой точке. Жди появления временного окна или возвращайся станционным транспортом.`,
        buttons: ['⬅️ Назад'],
      },
      nextState: {
        scene: SCENES.SHIP_TRAVEL,
  ...
  const lines = Object.entries(byDestination).map(([toId, variants]) => {
    ...
    const tempNote = variants[0].temporary
      ? ` (временный, ~${Math.round((variants[0].expiresAt - Date.now()) / 60000)} мин)`
      : '';
...
async function variantPickScreen(player, routesToDestination, destinationName) {
  const buttons = routesToDestination.map((r) => {
    const v = Object.values(ROUTE_VARIANTS).find(
      (rv) => rv.id === r.variant
    );

    return `${riskEmoji(r.riskLabel)} ${
      r.variant === 'dangerous'
        ? 'Опасный'
        : r.variant === 'safe'
          ? 'Безопасный'
          : 'Обычный'
    } (⛽${fuelCostForVariant(v)})`;
  });
...
/**
 * Один переход между узлами.
 */
async function resolveTransit(deps, player, route, rng) {
  const variant = Object.values(ROUTE_VARIANTS).find(
    (v) => v.id === route.variant
  );
  ...
  if (isPvpEvent && !variant.pvpAllowed) {
    return arriveAtNode(
      ...
      player,
      route.to,
      '🛰️ Безопасный маршрут прошёл спокойно.\n\n'
    );
  }
...
async function handleTravel(
  ...
  if (state.scene === SCENES.SHIP_TRAVEL) {
    if (input === '⬅️ Назад') {
      ...
      const node = nodeById(currentNodeId(player));
    ...
    if (state.pendingRoutes) {
      ...
      const route = state.pendingRoutes.find(
        (r) => r.variant === wantVariant
      );
    ...
    if (destMatch && state.availableRoutes) {
      const toNode = Object.values(NODES).find(
        (n) => n.name === destMatch[1]
      );
  ...
  if (state.scene === SCENES.SHIP_PRE_COMBAT) {
    ...
    const playerFighter = shipToFighter(
      player.ship,
      'Твой корабль'
    );
  ...
  if (state.scene === SCENES.SHIP_COMBAT) {
    ...
    if (result.defender.hp <= 0) {
      /*
       * ВАЖНО:
       * Победа в космическом бою НЕ означает возвращение в город.
       *
       * Сначала корабль прибывает именно в destinationNodeId.
       *
       * Если это location:
       *   остаёмся в SHIP_TRAVEL.
       *
       * Если это city:
       *   только тогда переходим в station и банкуем tripCargo.
       */
      return arriveAtNode(
        deps,
        player,
    ...
    if (enemyTurn.defender.hp <= 0) {
      const {
        lostTrip,
        lostInventory,
      } = loseFullCargo(player);

      player.ship.hp = Math.round(
      ...
      return {
        reply: {
          text: `⚔️ ${result.log.join(' ')} ${enemyTurn.log.join(' ')}\n\n💥 Корабль обездвижен. Спасательная капсула тянет тебя на станцию.${
            allLost.length
