...
// vein.js (по прямому запросу пользователя — рейд был единственным
// местом БЕЗ каких-либо последствий за смерть, ни личную, ни отрядную).
const EMERGENCY_POD_CHANCE = 0.12;
function applyRaidDefeatPenalty(player, rng) {
  if (rng() < EMERGENCY_POD_CHANCE) {
    return { text: '\n\n🛟 Аварийная капсула срабатывает — редкая удача, груз цел.', player: { ...player, hp: Math.round(player.hpMax * 0.3) } };
  }
  ...
  return { text: `\n\n💀 Поражение необратимо.${lossNote}`, player: { ...player, hp: 1 } };
}

const RAID_SLOT = 'default';
// ⚠️ БАГ-ФИКС (найден при тестировании личной карточки рейда):
// 'test_colossus' — несуществующий ID, findBoss() возвращал null для
// него ВСЕГДА, startRaidFromLobby() тоже — рейд физически не мог
// запуститься ни разу. Заменено на реального, существующего босса.
const RAID_BOSS_ID = 'ksarn_praxid';
...
async function raidBattleScreen(deps, player, playerId, raid, prefixText = '') {
  ...
  // ⚠️ Личная карточка статуса (по прямому запросу пользователя — раньше
  // был только один ряд в общем списке, не выделенная информация).
  const myCard = myMember ? renderStatusCard({
    title: 'ТВОЙ СТАТУС',
    rows: [{ label: 'HP', display: compactBar(myMember.hp, myMember.hpMax) }],
    meta: [`Позиция: ${myMember.row === 'front' ? '🛡️ передняя линия' : '🏹 тыл'}`, `Урон нанесён: ${myMember.damageDealt || 0}`],
    warnings: myMember.hp <= 0 ? ['💀 ТЫ ВЫБЫЛ ИЗ БОЯ'] : [],
  }) : '';
...
async function handleRaid(state, input, rng, deps, playerId) {
  ...
  if (state.scene === SCENES.RAID_BATTLE) {
    ...
    if (allActed(raid) || isTimedOut(raid)) {
      const wasAliveBeforeRound = (raid.members[playerId]?.hp || 0) > 0;
      const guildBonusMap = await buildGuildDamageBonusMap(deps, raid);
      resolveRound(raid, (id) => SKILLS[id], rng, guildBonusMap);
      await deps.raidStore.saveRaid(raid, RAID_SLOT);

      // ⚠️ Личная смерть ВНУТРИ продолжающегося рейда (отряд ещё бьётся
      // за остальных, но я лично выбыл) — раньше это было полностью
      // бесплатно (просто блокировка кнопки атаки). Применяем тот же
      // штраф, что и при общем поражении, но только ОДИН раз за этот
      // переход жив→мёртв, не при каждом повторном просмотре экрана, и
      // НЕ здесь, если рейд УЖЕ завершается целиком (тот случай ниже
      // обрабатывает штраф сам — не дублируем).
      if (!raid.finished && wasAliveBeforeRound && (raid.members[playerId]?.hp || 0) <= 0) {
        const penalty = applyRaidDefeatPenalty(state.player, rng);
        state.player = penalty.player;
        return raidBattleScreen(deps, state.player, playerId, raid, `💀 Ты выбыл из боя.${penalty.text}\n\n`);
      }
      ...
      if (raid.finished) {
        ...
        const penalty = applyRaidDefeatPenalty(state.player, rng);
        return {
          reply: { text: `💀 ОТРЯД ПОВЕРЖЕН\n\n${raid.log.join('\n')}\n\nБосс ещё стоит — можно собрать новый отряд.${penalty.text}`, buttons: stationButtons(deps, penalty.player) },
          nextState: { scene: 'station', player: penalty.player },
        };
      }
