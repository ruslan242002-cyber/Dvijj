...
function districtGroupsFor(
  ...
  // ⚠️ БАГ-ФИКС (найдено пользователем в реальной игре — районы
  // Кузницы вообще не прокликивались, любой клик падал в общий
  // статус станции): раньше здесь читался player.faction (домашняя
  // фракция) — но stationButtons() (чуть выше в этом же файле)
  // строит РЕАЛЬНО ПОКАЗЫВАЕМЫЕ игроку кнопки через currentStation()
  // (где игрок ФИЗИЧЕСКИ находится). Если игрок посещает станцию не
  // своей домашней фракции — эти два поля расходятся: показываются
  // кнопки станции визита, а совпадение искалось среди кнопок
  // домашней станции. Ни один label никогда не совпадал.
  const groups = (
    DISTRICT_GROUPS[
