...
async function pvpDuelScreen(deps, player, playerId, duelId, duelMaybe) {
  ...
  if (!myTurn) {
    return {
      reply: { text: `⚔️ Дуэль с ${opp.name}\n\nТы: ${me.hp}/${me.hpMax} — Соперник: ${opp.hp}/${opp.hpMax}\n\nСейчас ход противника — обнови позже.`, buttons: ['🔄 Обновить', '⬅️ Назад'] },
      nextState: { scene: 'pvp_duel', player, duelId },
    };
  }
  const buttons = ['⚔️ Обычная атака', ...(me.equippedSkills || []).map((id) => SKILLS[id]?.name).filter(Boolean), '⬅️ Назад'];
  return {
    reply: { text: `⚔️ Дуэль с ${opp.name}\n\nТы: ${me.hp}/${me.hpMax} — Соперник: ${opp.hp}/${opp.hpMax}\n\nТвой ход.`, buttons },
    nextState: { scene: 'pvp_duel', player, duelId },
  };
...
async function pvpHub(deps, player, playerId) {
  ...
  if ((player.level || 1) < PVP_MIN_LEVEL) {
    return {
      reply: { text: `⚔️ Дуэльная арена открывается с ${PVP_MIN_LEVEL} уровня — сейчас у тебя ${player.level || 1}.`, buttons: ['⬅️ Назад'] },
    ...
    case SCENES.PVP_MENU: {
      ...
      if (input === '🔍 Найти соперника') {
        if ((state.player.level || 1) < PVP_MIN_LEVEL) {
          return { reply: { text: `Дуэльная арена открывается с ${PVP_MIN_LEVEL} уровня.`, buttons: ['⬅️ Назад'] }, nextState: state };
        }
        try {
          ...
          return { reply: { text: 'Ты встал в очередь — при следующем заходе в «Дуэль» проверь, не нашёлся ли соперник.', buttons: ['⬅️ Назад'] }, nextState: { scene: 'station', player: state.player } };
        } catch (e) {
