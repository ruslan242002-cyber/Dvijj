...
return {
reply: { text: ` Дуэль с ${opp.name}\n\nТы: ${me.hp}/${me.hpMax} — Соперник: ${opp.hp}/${opp.hpMax}\n\nТвой ход:`, buttons },nextState: { scene: 'pvp_duel', player, duelId }
};
}
async function pvpHub(deps, player, playerId) {
if (!deps.pvpStore || !playerId) {
return { reply: { text: ' PvP сейчас недоступен.', buttons: stationButtons(deps, player) }, nextState: { scene: 'station', player } };
}
...
return {
reply: { text: ` Дуэльная арена открывается с ${PVP_MIN_LEVEL} уровня — сейчас у тебя ${player.level || 1}. Продолжай осваиваться, здесь тебя подождут.`, buttons: stationButtons(deps, player) },
...
addFactionReputation(player, player.faction, reward.reputation); // было: общее player.reputation — миграция на factionStanding
}
return {
reply: { text: ` Дуэль окончена. ${won ? `Победа! +${creditsGained} кредитов, +${reward.reputation} репутации.` : 'Поражение.'}`, buttons: stationButtons(deps, player) },
nextState: { scene: 'station', player }
};
}
