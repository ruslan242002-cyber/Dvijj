...
? `\n\n Корпус: ${p.ship.hp}/${p.ship.hpMax} — ремонт обойдётся в ${repairCost}${repairDiscount(p.faction) ? ' (со скидкой Арсенала)' : ' (3 кредита за HP)'}.`
: `\n\n Корпус: ${p.ship.hp}/${p.ship.hpMax} — в полном порядке.`;
...
const tankLine = `\n Расширение бака: +20 к ёмкости за Сплавы T2 ×15 и ${tankCost}${tankUpgradeDiscount(p.faction) ? ' (со скидкой Кузницы)' : ''}.`;
...
buttons.push(' Расширить бак');
for (const sys of damagedSystems) {
buttons.push(` Чинить: ${SYSTEM_NAMES[sys]}`);
}
...
if (input === 'Декон-камера') {const p = state.player;
const fee = deconFee(p.faction);
const feeLabel = fee === 0 ? ' Снять облучение (бесплатно)' : ` Снять облучение ( ${fee})`;
return {
reply: { text: ` ДЕКОН-КАМЕРА\n\nТекущее облучение: ${p.radiation || 0}%${p.radiation ? `\nСтоимость очистки: ${fee === 0 ? 'бесплатно' : ` ${fee}`}` : ''}`, buttons: p.radiation ? [feeLabel, ' Назад'] : [' Назад'], imageKey: imageForLocation('decon', p.faction) },
...
: '';
return { reply: { text: ` ВРАТА ТРАКТА\n\nКуда проложить курс?${lockedNote}`, buttons: [...unlocked, ' Назад'], imageKey: imageForLocation('gates', state.player.faction) }, nextState: { scene: 'loc_gates', player: state.player } };
}
...
const player = { ...state.player, flags: { ...(state.player.flags || {}) } };const firstVisit = !player.flags.visited_memory_terrace;
...
return {
reply: { text: `${prefix} ${group.label}`, buttons: [...group.buttons, ' Назад'] },nextState: { scene: 'district_hub', player, groupLabel: group.label }
};
}
