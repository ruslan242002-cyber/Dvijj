...
const {
  ...
  tankUpgradeDiscount,
  importMarkup,
  TANK_UPGRADE_CREDITS,
  TOOL_COSTS,
} = require('./locations/repair.js');
const {
  SHIP_SYSTEMS,
  SYSTEM_NAMES,
  freshShipSystems,
  ...
  repairSystem,
  repairCostForSystem,
} = require('../../engine/ship-systems.js');
const {
  findSkin,
  skinsAvailableFor,
  ownedSkins,
...
async function resolveStationAction(input, state, deps, rng, playerId) {
  ...
  if (input === 'Профиль') {
    ...
    if (!link) {
      return {
        reply: {
          text: 'Терминал профиля сейчас недоступен, попробуйте позже.',
          buttons: stationButtons(deps, state.player),
        },
        nextState: {
          scene: 'station',
  ...
  if (input === 'Отсек') {
    ...
    const items = (p.inventory || [])
      .map((i) => `${i.resource} T${i.tier} ×${i.qty}`)
      .join(', ');
    ...
    const repairCost = Math.round(
      missingHp *
        3 *
        (1 - repairDiscount(currentStation(p))) *
        (1 + importMarkup(currentStation(p)))
    );
    ...
    const tankCost = Math.round(
      TANK_UPGRADE_CREDITS *
        (1 - tankUpgradeDiscount(currentStation(p)))
    );

    const shipLine =
      missingHp > 0
        ? `\n\n🚀 Корпус: ❤️ ${p.ship.hp}/${p.ship.hpMax} — ремонт обойдётся в 💳${repairCost}${
            repairDiscount(currentStation(p))
              ? ' (со скидкой Арсенала)'
              : ' (3 кредита за HP)'
          }.`
    ...
    const tankLine = `\n🛢️ Расширение бака: +20 к ёмкости за Сплавы T2 ×15 и 💳${tankCost}${
      tankUpgradeDiscount(currentStation(p))
        ? ' (со скидкой Кузницы)'
        : ''
    }.`;
    ...
    const damagedSystems = SHIP_SYSTEMS.filter(
      (sys) => p.ship.systems[sys] < 100
    );
    ...
    const toolLines = Object.entries(TOOL_COSTS)
      .map(
        ([id, cost]) =>
          `\n🔩 ${cost.name}: ${
            ownedTools.includes(id)
              ? 'уже есть'
              : `${cost.resource} T${cost.tier} ×${cost.qty} + 💳${cost.credits}`
          }`
      )
      .join('');
    ...
    const skinsLine = `\n\n🎨 Окраска корабля (${
      findSkin(equippedSkinId)?.name || 'Стандартный корпус'
    }):\n${availableSkins
      .map(
        (s) =>
          `${mySkins.includes(s.id)
            ? s.id === equippedSkinId
              ? '✅'
              : '◻️'
    ...
    if (items) {
      buttons.push('🧹 Продать лишнее', '💰 Продать всё');
    }

    if (missingHp > 0) {
      buttons.push(`🔧 Ремонт (💳${repairCost})`);
    }
    ...
    if (
      ...
      buttons.push(`⛽ Заправка (💳${fuelCost})`);
    }
    ...
    for (const [id, cost] of Object.entries(TOOL_COSTS)) {
      if (!ownedTools.includes(id)) {
        buttons.push(`Купить: ${cost.name}`);
      }
    }
    ...
    return {
      reply: {
        ...
        }${shipLine}${fuelLine}${systemsLine}${tankLine}${skinsLine}${toolLines}`,
        buttons,
        imageKey: imageForLocation(
          'repair',
          currentStation(p)
        ),
      },
  ...
  if (input === 'Декон-камера') {
    ...
    const feeLabel =
      fee === 0
        ? '☢️ Снять облучение (бесплатно)'
        : `☢️ Снять облучение (💳${fee})`;
  ...
  if (input === 'Бар') {
    if (state.player.visitingStation) {
      return {
        reply: {
          text: `Куратор ${state.player.visitingStation} не станет говорить с чужаком. Чтобы попасть в бар, нужно вступить во фракцию (Мостик → Станция приписки).`,
          buttons: stationButtons(deps, state.player),
        },
        nextState: {
          scene: 'station',
  ...
  if (input === 'Контракты') {
    if (state.player.visitingStation) {
      return {
        reply: {
          text: 'Доска контрактов доступна только участникам фракции.',
          buttons: stationButtons(deps, state.player),
        },
        nextState: {
          scene: 'station',
  ...
  if (input === 'Архив теней') {
    return {
      reply: {
        text: 'Скрытные вылазки временно доступны только через полёт — набери «Полёт» на хабе и выбери скрытную высадку там, когда доберёшься до планеты.',
  ...
  if (input === 'Врата Тракта') {
    const others = FACTIONS.filter(
      (f) => f !== state.player.faction
    );

    const level = state.player.level || 1;

    const unlocked = others.filter(
      (f) => level >= (CITY_UNLOCK_LEVEL[f] || 0)
    );

    const locked = others.filter(
      (f) => level < (CITY_UNLOCK_LEVEL[f] || 0)
    );
    ...
    return {
      reply: {
        text: `🌀 ВРАТА ТРАКТА\n\nКуда проложить курс?${lockedNote}`,
        buttons: [...unlocked, '⬅️ Назад'],
        imageKey: imageForLocation(
          'gates',
          currentStation(state.player)
        ),
      },
      nextState: {
        scene: 'loc_gates',
  ...
  if (input === 'Исследовать') {
    return {
      reply: {
        text: 'Разведка теперь начинается с полёта — набери «Полёт» на хабе, долети до планеты и высадись там.',
        buttons: stationButtons(
          deps,
          state.player
        ),
  ...
  if (input === 'Полёт') {
    return travelScreen(
      deps,
      state.player,
      '🚀 Врата открываются — курс в открытый космос.\n\n'
    );
  }
  ...
  if (input === 'Терраса памяти') {
    ...
    const firstVisit =
      !player.flags.visited_memory_terrace;
    ...
    return {
      reply: {
        text: `🪟 ТЕРРАСА ПАМЯТИ\n\nТа самая смотровая палуба, где всё началось. Новички задерживаются здесь дольше остальных — кто-то ищет ответы во внешнем Тракте, кто-то просто привыкает к тишине после капсулы. Ветераны заходят реже, но заходят.${bonusNote}`,
        buttons: stationButtons(
          deps,
          player
        ),
  ...
  if (input === 'Мастерская новичка') {
    ...
    const firstVisit = !(
      player.flags || {}
    ).visited_novice_workshop;

    player.flags = {
      ...(player.flags || {}),
      visited_novice_workshop: true,
    };
    ...
    return {
      reply: {
        text: '🛠️ МАСТЕРСКАЯ НОВИЧКА\n\nИнструктор кивает — ты уже был(а) здесь. Дальше сам(а), с настоящим риском, как у всех.',
        buttons: stationButtons(
          deps,
          player
        ),
  ...
  if (input === 'Барак ожидания') {
    return {
      reply: {
        text: '🛌 БАРАК ОЖИДАНИЯ\n\nРяды коек для тех, кто ещё не выбрал станцию — или выбрал, но не спешит туда переезжать. Здесь тихо обмениваются слухами о том, как оно там, у соседей. Хорошее место, чтобы никуда не торопиться.',
        buttons: stationButtons(
          deps,
          state.player
        ),
  ...
  if (input === '⛏️ Жила') {
    if (!deps.veinStore) {
      return {
        reply: {
          text: '⛏️ Система жил сейчас недоступна.',
          buttons: stationButtons(
            deps,
            state.player
          ),
    ...
    const vein =
      await deps.veinStore.getActiveVein();

    if (!vein) {
      return {
        reply: {
          text: '⛏️ Активной жилы сейчас нет. Как только она появится — тебе придёт уведомление, где бы ты ни был(а).',
          buttons: stationButtons(
            deps,
            state.player
          ),
    ...
    return {
      reply: {
        text: `⛏️ ЖИЛА ОБНАРУЖЕНА\n\nТ${vein.tier} · ${vein.resource}\nПрочность: ${Math.round(
          (vein.durability /
            vein.durabilityMax) *
            100
        )}%\nМестоположение: дистанция ${vein.distance} (${zoneName})\n\nНабери «Полёт» и долети до этой дистанции — на месте появится возможность пристыковаться.`,
        buttons: stationButtons(
          deps,
          state.player
        ),
...
function stationDefaultView(
  deps,
  player,
  rng
) {
  const card = stationArrivalCard(
    player,
    rng
  );
  ...
  if (card.reward) {
    ...
    if (card.reward.credits) {
      nextPlayer.credits =
        (nextPlayer.credits || 0) +
        card.reward.credits;
    }
  ...
  const loginResult =
    checkDailyLogin(nextPlayer);

  let dailyNote = '';

  if (loginResult.rewarded) {
    const resourceNote =
      loginResult.reward.resource
        ? ` + ${loginResult.reward.resource.qty}× ${loginResult.reward.resource.resource} T${loginResult.reward.resource.tier}`
        : '';
    ...
    case SCENES.STATION: {
      ...
      if (group) {
        ...
        if (stationEvent?.reward) {
          ...
          if (
            stationEvent.reward.reputation
          ) {
            addFactionReputation(
              player,
              player.faction,
