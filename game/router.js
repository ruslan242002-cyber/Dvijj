...
// Команда "Домой" — по разбору бага со спам-кликом: аварийный выход,
// работающий НЕЗАВИСИМО от того, в какой сцене застрял игрок и сломан
// ли конкретный обработчик сцены. В отличие от RESET_COMMAND ("Сброс")
// НЕ стирает прогресс — просто безусловно возвращает на станцию с
// текущим player как есть. Проверяется раньше диспетчеризации по сценам,
// тем же способом, что и "Сброс".
const HOME_COMMAND = '/домой';
async function step(state, text, rng = Math.random, deps = {}, playerId = null) {
const input = (text || '').trim();
if (input === RESET_COMMAND) {
return RESET_REPLY;
}
if (input === HOME_COMMAND) {
const player = state?.player;
if (!player) return RESET_REPLY;
return {
reply: { text: ' Возвращаю тебя на станцию.', buttons: [] },
nextState: { scene: SCENES.STATION, player },
};
}
...
FACTIONS, FACTION_KIT, CURATORS, MAX_EQUIPPED_SKILLS, ZONE_BUTTONS, MIN_LEVEL_FOR_ZONE, RESET_COMMAND, HOME_COMMAND,
SCENES,
};
