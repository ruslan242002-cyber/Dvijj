...
* (8/15/22/30), завязанных на сквозную интригу "Пятый Голос". Кроме* куратора каждой станции, квесты дают ещё двое именных персонажей —
...
/** Визуальный прогресс-бар поверх progressText() — по разбору Kimi.
* Работает для всех типов objective (deliver/kill/explore), т.к. все
* они уже сводятся к паре "текущее/цель" в progressText(). */
function progressBar(player, o) {
const raw = progressText(player, o);
const [current, target] = raw.split('/').map(Number);
if (!target) return raw;
const filled = Math.max(0, Math.min(10, Math.round((current / target) * 10)));
return `[${'█'.repeat(filled)}${'░'.repeat(10 - filled)}] ${raw}`;
}
...
const NPCS = {'Приют': ['Ирис Вейл', 'Техник Дорн', 'Сестра Мира'],
...
title: 'Материал для маскировки',text: 'Аналитик Рю, не поднимая головы от осциллографа: «Нужны сплавы почище для маскировочных контуров — восемь кусков, не больше и не меньше. Точность важна».',
...
objective: { type: 'explore', zone: 'blue', count: 3 },
reward: { xp: 30, credits: 40 } },{ id: 'vual_2', city: 'Вуаль', minLevel: 5,
title: 'Сбои на линии',
...
module.exports = { QUESTS, NPCS, availableQuests, getQuest, describeObjective, progressText, progressBar, objectiveMet, consumeObjective };
