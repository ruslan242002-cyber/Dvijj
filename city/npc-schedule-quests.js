'use strict';
/**
 * КВЕСТЫ ПО РАСПИСАНИЮ NPC — раньше `schedule` в city/npc-roster.js был
 * чистым флейвором (что NPC делает в разное время суток), без единой
 * механики поверх. Здесь — по одному квесту на каждого из 16 NPC,
 * привязанному к конкретному слоту их же расписания.
 *
 * ИСПРАВЛЕНО ПРОТИВ АРХИВНОЙ ВЕРСИИ: поле city у 6 квестов (Илва/Брок/
 * Тайлер и Кес/Рю/Орен) было перепутано — Терминус и Вуаль оказались
 * местами, потому что архивная версия писалась по тому же устаревшему
 * полю station в npc-roster.js, которое я уже нашёл и починил раньше
 * (см. city/npc-roster.js: npcsForStation). Сверено с авторитетным
 * city/districts-data.js: Терминус — shyopot/osvedomitel_kes/
 * analitik_ryu/slushatel; Вуаль — drogo_keyn/serzhant_ilva/mehanik_brok/
 * novobranets.
 *
 ...
 * только когда игрок посещает станцию этого NPC в соответствующий слот
 * расписания (morning/day/evening/night — те же ключи, что в
 * STATION_SHIFTS/schedule).
 */
const NPC_SCHEDULE_QUESTS = [
  ...
  // ── Терминус (shyopot/osvedomitel_kes/analitik_ryu/slushatel) ──
  ...
  { id: 'sq_kes_evening', npc: 'osvedomitel_kes', slot: 'evening', city: 'Терминус', minLevel: 3,
  ...
  { id: 'sq_ryu_night', npc: 'analitik_ryu', slot: 'night', city: 'Терминус', minLevel: 6,
  ...
  { id: 'sq_oren_evening', npc: 'slushatel', slot: 'evening', city: 'Терминус', minLevel: 10,
  ...
  // ── Вуаль (drogo_keyn/serzhant_ilva/mehanik_brok/novobranets) ──
  ...
  { id: 'sq_ilva_morning', npc: 'serzhant_ilva', slot: 'morning', city: 'Вуаль', minLevel: 4,
  ...
  { id: 'sq_brok_morning', npc: 'mehanik_brok', slot: 'morning', city: 'Вуаль', minLevel: 3,
  ...
  { id: 'sq_tayler_evening', npc: 'novobranets', slot: 'evening', city: 'Вуаль', minLevel: 2,
    title: 'Байки бывалых',
...
/** Квест по расписанию доступен, если: это тот NPC, сейчас его слот
 * времени, игрок физически на его станции (currentStation — можно и в
 * гостях, это не куратор-завязанный контент), уровень достаточен, и
 * квест ещё не выполнен и не взят активно. */
function getAvailableScheduleQuest(player, npcId, currentSlot, station) {
  const quest = NPC_SCHEDULE_QUESTS.find((q) => q.npc === npcId && q.slot === currentSlot && q.city === station);
  if (!quest) return null;
  if ((player.level || 1) < quest.minLevel) return null;
  if ((player.completedQuests || []).includes(quest.id)) return null;
  return quest;
}

module.exports = { NPC_SCHEDULE_QUESTS, getAvailableScheduleQuest };
