...
/**
 * ЛИЧНОЕ ЖИЛЬЁ — не только флейвор: у каждой станции свой пассивный бонус,
 * который включается сразу при покупке дома, плюс 2 предмета интерьера на
 * станцию, каждый добавляет ещё немного того же эффекта. Всё с потолком
 * (см. PERK_CAPS ниже) — дом не должен становиться обязательным силовым
 * множителем, только заметным, но ограниченным преимуществом.
 *
 ...
 * getCraftDiscount(player), но не подключена ни к какому реальному
 * crafting-engine.js — формулу нужно вызвать из реального расчёта
 * стоимости крафта. Скидка на биржу и бонус к эвакуации уже подключены
 * и протестированы (см. market-engine.js и evacuation.js).
 *
 * ПРОВЕРЬ: часть файла (flavor-тексты и amount у предметов интерьера)
 * была обрезана при экспорте в PDF и восстановлена мной по смыслу —
 * помечено ниже отдельно. Логика и структура (поля, функции, ошибки,
 * module.exports) — из архива один в один, не трогал.
 */
...
const HOUSING = {
  'Приют': {
    ...
    // ПРОВЕРЬ: конец фразы обрезан в архиве, дописал по смыслу
    flavor: 'Тесная, но своя — над головой вечный гул грузовых лифтов Приюта, зато соседи все свои, торговый ряд под боком.',
  ...
  'Терминус': {
    ...
    // ПРОВЕРЬ: конец фразы обрезан в архиве, дописал по смыслу
    flavor: 'Без окон и почти без света — как и полагается в городе-тени. Дверь запирается на три засова, и это радует.',
  ...
  'Арсенал': {
    ...
    // ПРОВЕРЬ: конец фразы обрезан в архиве, дописал по смыслу
    flavor: 'Стены вибрируют от работы станков через переборку. Спать тяжело первую неделю — потом привыкаешь и уже не можешь без этого гула.',
  ...
  'Вуаль': {
    ...
    // ПРОВЕРЬ: конец фразы обрезан в архиве, дописал по смыслу
    flavor: 'Компактный, но со своей системой климат-контроля — редкая роскошь. Мастерская Вуали в двух шагах.',
    perk: { type: 'craft_discount', base: 0.1 }, // −10% стоимости крафта (не подключено, см. шапку файла)
  },
};
...
// ПРОВЕРЬ: amount у каждого предмета обрезан в архиве. Подобрал так, чтобы
// база дома + оба предмета ровно упирались в PERK_CAPS этой станции —
// поправь под реальные цифры, если они были другие.
const HOUSE_ITEMS = {
  'Приют': [
