...
async function handleProfileRequest({ method, token, launchParams, body }, { store, secret, vkAppSecret }) {
  ...
  if (method === 'POST') {
    ...
    if (action === 'allocateStat') {
      ...
      // Пересчитываем производные боевые статы (уворот/меткость/фокус/HP/
      // урон/броню) сразу после траты очка — см. engine/derived-stats.js.
      // Без этого вызова потраченное очко просто лежало бы в stats[stat]
      // и НИКАК не влияло бы на реальный бой.
