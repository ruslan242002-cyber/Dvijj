...
const { verifyLaunchParams, parseLaunchParams, getUserId } = require('./vk-launch-params.js');
...
/**
 * Определяет, чей это профиль. Сначала пробует параметры запуска VK Mini App
 * (launchParams — сырая query-строка вида "vk_user_id=...&sign=..."), это
 * основной путь при открытии как мини-приложения ВК. Если их нет или секрет
 * мини-аппа не настроен — используется старая подписанная ссылка (token),
 * оставленная для обратной совместимости и локальных тестов.
 */
function resolvePeerId({ token, launchParams }, { secret, vkAppSecret }) {
  if (launchParams && vkAppSecret) {
    const parsed = typeof launchParams === 'string' ? parseLaunchParams(launchParams) : launchParams;
    if (verifyLaunchParams(parsed, vkAppSecret)) return getUserId(parsed);
  }
  if (token) return verify(token, secret);
  return null;
}

async function handleProfileRequest({ method, token, launchParams, body }, { store, secret, vkAppSecret }) {
  const peerId = resolvePeerId({ token, launchParams }, { secret, vkAppSecret });
  if (!peerId) return { status: 401, json: { error: 'Не удалось подтвердить пользователя ВК. Откройте профиль заново из игры.' } };
...
module.exports = { handleProfileRequest, resolvePeerId, skillCatalog, ALLOCATABLE_STATS };
