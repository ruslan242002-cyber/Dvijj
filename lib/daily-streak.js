...
* daily-seeded паттерна из contracts-engine.js (тот же DAY_MS/getDaySeed
* подход, не отдельный таймер). Награда растёт с длиной серии и
* сбрасывается, если пропущен день (не просто "давно не заходил", а
* конкретно предыдущий календарный день пуст).
*
* checkDailyLogin(player, now) — вызывать один раз при первом действии
* игрока за день (например, там же, где router.js уже определяет смену
* дня для контрактов). Мутирует player, возвращает { rewarded, streak,
