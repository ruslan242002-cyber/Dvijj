...
function combatStatusCard(fighter, { icon = '❤️', prevHp = null } = {}) {
  const bar = progressBar(fighter.hp, fighter.hpMax);
  let delta = '';
  if (prevHp !== null && prevHp !== undefined) {
    const diff = fighter.hp - prevHp;
    delta = ` (${diff > 0 ? '+' : ''}${diff})`;
  }
  ...
  const firepower = fighter.stats?.firepower ?? 0;
  const shielding = fighter.stats?.shielding ?? 0;

  return (
    `${icon} ${fighter.name}: ${fighter.hp}/${fighter.hpMax}\n${bar}${delta}\n` +
