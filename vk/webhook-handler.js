...
async function handleVkEvent(body, deps) {
  const {
    store, vk, rng = Math.random, confirmationCode, secret, getProfileLink, resolveEnemyImage,
    knownPlayersStore, redis,
  } = deps;
  ...
  if (body.type === 'message_new') {
    ...
    // ИСПРАВЛЕНО: раньше сюда попадала только часть сторов из deps
    // вручную перечисленным списком (marketStore/pvpStore/ambushStore/
    // veinStore/bossStore/raidStore/guildStore/redis/store) —
    // knownPlayersStore/worldStateStore/tractStore/wreckageStore/
    // leaderboardStore молча терялись здесь и были undefined в любой
    // сцене, даже если api/vk.js их правильно создавал. Теперь
    // routerDeps прокидывает ВСЁ, что реально пришло в deps — ничего не
    // перечисляем вручную повторным списком, чтобы это не повторилось
    // при добавлении следующего нового стора.
    const routerDeps = {
      ...deps,
