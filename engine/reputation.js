...
* player.factionStanding[faction] (объект по станциям), которое уже
* существовало параллельно (choices/consequence-engine.js,
* lore/trakt-mythos.js unlockCondition: 'faction_reputation').
*
...
* флагом player.reputationMigrated, чтобы не переносить повторно (на
* случай, если player.reputation зачем-то останется в объекте).
* Вызывать в начале обработки любого запроса игрока (router.js,
* рядом с остальной санитацией state.player), до любого чтения
* репутации.
*/
