...
function rollSpaceEvent(player, distance, rng = Math.random, ambushContext = null) {
  ...
  switch (kind) {
    case 'empty_space':
      return { type: 'empty_space', text: 'Пустой участок пространства. Двигатели гудят ровно, приборы молчат.' };

    case 'hostile_ship': {
      const enemy = generateHostileShip(distance, player.shipLevel || 1, rng);
      return { type: 'hostile_ship', enemy, text: `⚠️ На сканере — ${enemy.name}. Курс на пересечение.` };
    }

    case 'derelict_wreck': {
      const loot = rollSpaceLoot(distance, rng);
      return { type: 'derelict_wreck', loot, text: `Дрейфующие обломки корабля — старые, но трюм цел. Забираешь ${loot.qty}× ${loot.resource} T${loot.tier}.` };
    }

    case 'distress_signal': {
      const reward = Math.round((30 + distance * 6));
      return { type: 'distress_signal', reward: { credits: reward }, text: 'Слабый сигнал бедствия на аварийной частоте. Кто-то просит о помощи.' };
    }

    case 'asteroid_field': {
      const loot = rollSpaceLoot(distance, rng);
      const hullRisk = 0.25; // шанс повредить корпус при добыче
      return { type: 'asteroid_field', loot, hullRisk, text: 'Астероидное поле, богатое рудой — можно рискнуть и добыть, но обшивке достанется.' };
    }

    case 'space_anomaly': {
      const fuelDrain = Math.round(6 + distance * 1.5);
      return { type: 'space_anomaly', fuelDrain, text: `Искажение Тракта закручивает пространство вокруг корабля — двигатели жгут топливо впустую (-${fuelDrain}).` };
    }
    ...
    case 'patrol_greeting':
      return { type: 'patrol_greeting', text: 'Патрульный катер станции сверяет позывной и отворачивает — путь свободен.' };

    case 'gravity_anomaly': {
      const fuelDrain = Math.round(10 + distance * 2);
      return { type: 'gravity_anomaly', fuelDrain, text: `Неучтённая гравитационная линза утягивает корабль в сторону — приходится жечь топливо на коррекцию курса (-${fuelDrain}).` };
    }

    default:
      return { type: 'empty_space', text: 'Пустой участок пространства.' };
  }
}
