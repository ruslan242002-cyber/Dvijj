...
// critModifier (engine/skills-data.js) — множитель к базовому шансу крита
// для конкретного умения (по разбору доп. улучшений: некоторые умения
// логично критуют чаще своей резкой природы). 1, если не задан.
const isCrit = rng() < critChance(attacker.luck ?? 0) * (skill.critModifier ?? 1);
...
/**
* ЗОНАЛЬНЫЕ МОДИФИКАТОРЫ БОЯ — добавлено по разбору Kimi. Жёлтая зона:
* 10% шанс "помехи" (сенсоры сбоят, -10% точности ОБОИМ на этот ход).
* Красная зона: 20% шанс "резонанса" (случайный урон 5-15 по ОБОИМ,
* не зависит от щита/брони — это фон, а не атака). Синяя зона — без
* модификаторов. Один ролл на ход, применяется в resolveTurn() для
* и игрока, и врага одинаково (единая точка входа).
*/
...
function resolveTurn({ attacker, defender, stim, skill, zone, rng = Math.random }) {
const log = [];
