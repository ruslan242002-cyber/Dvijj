...
/** Пожертвовать РЕСУРСЫ в банк гильдии — по разбору доп. улучшений:
* раньше банк принимал только кредиты, а гильдия существует в первую
* очередь ради совместного крафта, для которого нужны именно ресурсы.
* Снятие из инвентаря — обычная мутация игрока (это его стак), запись в
* общий банк — атомарная операция в сторе (тот же паттерн, что кредиты). */
...
/** Забрать ресурсы из банка гильдии — только offiter/leader (простая
